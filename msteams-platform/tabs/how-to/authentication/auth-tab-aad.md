...
>[!NOTE]
>To get the correct token for Microsoft Teams Free and guest users, it is important that the apps use [tenantId](https://login.microsoftonline.com/{tenantId}), which you can get from the bot message or tab context. If the apps use [common](https://login.microsoftonline.com/common), the users will get the incorrect tokens and will log on to the "home" tenant instead of the tenant that they are currently signed into. 
